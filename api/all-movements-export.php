...
// Access control - only show movements from accessible branches
if (!$isSuperAdmin) {
...
// Generate Excel file
$filterParts = [];
if ($productId > 0) $filterParts[] = 'prod' . $productId;
if ($warehouseId > 0) $filterParts[] = 'wh' . $warehouseId;
if ($movementType !== '') $filterParts[] = 'type-' . $movementType;
if ($dateFrom !== '') $filterParts[] = 'from-' . str_replace('-', '', $dateFrom);
if ($dateTo !== '') $filterParts[] = 'to-' . str_replace('-', '', $dateTo);

$filename = 'all-movements-' . (empty($filterParts) ? 'full-platform' : implode('-', $filterParts)) . '-' . date('Y-m-d-His') . '.xls';
...
foreach ($rows as $row) {
    ...
    $totalValue = (($row['quantity_in'] ?? 0) - ($row['quantity_out'] ?? 0)) * ($row['unit_cost'] ?? 0);
    ...
    echo '<td>' . e($row['warehouse_code'] ?? '') . '</td>';
    echo '<td>' . e($row['warehouse_name'] ?? '-') . '</td>';
    echo '<td>' . e($row['branch_name'] ?? '-') . '</td>';
    echo '<td>' . e($movementTypeLabel) . '</td>';
    echo '<td>' . e($reference) . '</td>';
    echo '<td style="mso-number-format:\'0.000\';">' . ($row['quantity_in'] > 0 ? e(format_number($row['quantity_in'], 3)) : '') . '</td>';
    echo '<td style="mso-number-format:\'0.000\';">' . ($row['quantity_out'] > 0 ? e(format_number($row['quantity_out'], 3)) : '') . '</td>';
    echo '<td style="mso-number-format:\'0.00\';">' . e(format_currency($row['unit_cost'] ?? 0)) . '</td>';
    echo '<td style="mso-number-format:\'0.00\';">' . e(format_currency($totalValue)) . '</td>';
    echo '<td>' . e($row['notes'] ?? '') . '</td>';
    echo '<td>' . e($row['created_by_name'] ?? '-') . '</td>';
